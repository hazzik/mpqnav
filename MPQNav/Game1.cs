...
namespace MPQNav {
	...
	public class Game1 : Game {
		...
		private readonly ADTManager manager;
		...
		public Game1() {
			...
			String mpqPath = ConfigurationSettings.AppSettings["mpqPath"];
			//String mpqFile = "c:\\Program Files\\World of Warcraft\\Data\\common.MPQ";
			String defaultContinent = ConfigurationSettings.AppSettings["defaultContinent"];
			int defaultMapX = int.Parse(ConfigurationSettings.AppSettings["defaultMapX"]);
			int defaultMapY = int.Parse(ConfigurationSettings.AppSettings["defaultMapY"]);

			var continent = (ADTManager.ContinentType)Enum.Parse(typeof(ADTManager.ContinentType), defaultContinent, true);

			manager = new ADTManager(continent, mpqPath);

			manager.loadADT(defaultMapX, defaultMapY);

			renderIndices = manager.renderingIndices();
			renderVerticies = manager.renderingVerticies();
			if(renderIndices.Count > 0) {
		...
		protected override void LoadContent() {
			...
			var thaFont = Content.Load<SpriteFont>(@"mfont");
			spriteFont = thaFont;
		...
		private void InitializeEffect() {
			...
			vertexDeclaration = new VertexDeclaration(graphics.GraphicsDevice, VertexPositionNormalColored.VertexElements);
		}
