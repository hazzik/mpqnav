...
namespace MPQNav.Collision {
	internal class Collider {
		...
		public MPQNav.Collision._3D.OBB characterOBB = new MPQNav.Collision._3D.OBB(new Vector3(0, .8f, 0),
		...
		public List<Vector3> getStandablePoints(Vector3 startPoint, float resolution, float max_x, float max_z,
		                                        VertexPositionNormalColored[] triangleVertexList, short[] triangleIndexList) {
		...
		public List<Vector3> getStandablePoints(Vector3 startPoint, float resolution, float max_x, float max_z,
		                                        List<VertexPositionNormalColored> triangleVertexList,
		                                        List<short> triangleIndexList) {
			List<Vector3> vList = new List<Vector3>();
		...
		public List<Vector3> getStandablePoints(Vector3 startPoint, float resolution, float max_x, float max_z,
		                                        List<Vector3> triangleVertexList, List<int> triangleIndexList) {
			...
			for(int z = 0; z < 5;) {
				...
				for(float x = startPoint.X; x > max_x; x -= resolution) {
					...
					while(count < triangleIndexList.Count / 3 && found == false) {
						...
						float t, u, v;
						if(MPQNav.Collision._2D.Triangle.RayTriangleIntersect(new Vector3(x, 0, z), Vector3.Down, p1, p2, p3, out t, out u,
						                                                      out v)) {
							found = true;
						...
						else if(MPQNav.Collision._2D.Triangle.RayTriangleIntersect(new Vector3(x, 0, z), Vector3.Up, p1, p2, p3, out t, out u,
						                                                           out v)) {
							found = true;
							intersectionPoint.X = x;
							intersectionPoint.Z = z;
				...
				sw.Stop();
				step++;
				Console.WriteLine((float)((float)step / (float)steps) * 100 + "% Done - " +
				                  (TimeSpan.FromTicks(sw.ElapsedTicks * (steps - step))) + " to go");
				sw.Reset();
				return returnList;
			}
			return returnList;
